...
export default function PDFUploader({
  ...
  const uploadSinglePdf = async (file: any) => {
    ...
    const res = await axios.post(
      `${import.meta.env.VITE_BASE_URI}/upload`,
      data,
      {
        headers: {
  ...
  function selectAsMainPhoto(ev: any, filename: any) {
    ...
    onChange([
      filename,
      ...addedPdfs.filter((addedPdf) => addedPdf !== filename),
    ]);
  }
    ...
    <>
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {addedPdfs.length > 0 &&
          addedPdfs.map((link) => (
            <div
              className="h-32 bg-gray-100 rounded-xl flex relative min-w-[130px]"
              key={link}
            >
              <CgNotes className="w-12 h-12 m-auto text-gray-500" />
              <button
                type="button"
                onClick={(ev) => removePdf(ev, link)}
